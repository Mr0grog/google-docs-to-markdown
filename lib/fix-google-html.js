...
import { wrapChildren } from './hast-tools.js';
import {
  sliceClipText,
...
/**
 * Find paragraphs that represent actual code block objects in a Google Doc
 * (only available on business and enterprise plans) using the slice clip data
 * and mark up the relevant paragraphs as code.
 *
 * We also handle things that are code-block-like (e.g. consecutive lines of
 * monospace-only text) separately in `createCodeBlocks`, but this lets us pull
 * extra data from the slice clip when available, or handle inconsistent
 * formatting inside the block, which we otherwise could not.
 *
 * @param {HastNodes} tree
 * @param {any} sliceClip
 */
function fixCodeBlockObjects(tree, sliceClip) {
  if (!sliceClip) return;

  const textRanges = rangesForCodeBlocks(sliceClip);

  // Convert the text ranges to ranges covering the lines (paragraphs) in HAST.
  ...
  // Go in reverse order so we can use the indexes as is, without worrying about
  // how replacing each block changes the indexes of the next one.
  for (const block of treeRanges.reverse()) {
    const length = block.end - block.start;
    const contents = block.node.children
      .slice(block.start, block.end)
      .flatMap((node) =>
        // Unwrap paragraphs and replace them with their contents + a line break
        // so we don't wind up adding blank lines around each line of code.
    ...
    const languageClasses = block.codeBlock.language
      ? [`language-${block.codeBlock.language}`]
      : [];
    block.node.children.splice(block.start, length, {
      type: 'element',
      tagName: 'pre',
      properties: {},
      children: [
...
export default function fixGoogleHtml() {
  return (tree, _file) => {
    ...
    fixCodeBlockObjects(tree, _file.data.sliceClip);
