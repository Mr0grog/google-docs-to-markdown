...
const tableStructureElements = new Set([
  'thead',
  'tbody',
  'th',
  'td',
...
const isTableComponent = (node) => tableStructureElements.has(node.tagName);
 ...
 * @param {RehypeNode} node Fix the tree below this node
 */
export function mergeConsecutiveInlineStyles (node) {
  visit(node, n => n.tagName, (node, _index, _parent) => {
    const newChildren = [];
    let activeNode = null;
    for (const child of node.children) {
      ...
      if (!activeNode) {
        ...
        if (child.tagName && !isBlock(child) && !isTableComponent(child)) {
          activeNode = child;
        }
      }
    }
...
export default function fixGoogleHtml () {
  return (tree, _file) => {
    unInlineStyles(tree);
    mergeConsecutiveInlineStyles(tree);
