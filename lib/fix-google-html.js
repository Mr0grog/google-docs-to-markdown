...
const log = debug('app:fix-google-html:log')
const error = debug('app:fix-google-html:error')
 ...
 * @param {RehypeNode} node 
 * @param {*} rawInternalContent 
 */
function fixInternalLinks (node, rawInternalContent) {

  // get the internal headings
  const internalContent = tryCatch(() => JSON.parse(JSON.parse(rawInternalContent).data).resolved)
  ...
  if (!internalHeadings || internalHeadings.length === 0) return;

  const headings = [];
  visit(node, n => n.tagName && /^h\d$/.test(n.tagName), (node, _index, _parent) => {
    headings.push(node);
  });
  ...
  headings.forEach((heading, index) => {
    ...
    const rawNewId = slug(getTextContent(heading));
    let count = 0;
    while (true) {
      const newId = count === 0 ? rawNewId : `${rawNewId}-${count}`;
      const exists = headings.some(heading => heading.properties.id === newId);
      if (!exists) {
  ...
  visit(node, n => n.tagName === 'a', (node, _index, _parent) => {
    ...
    if (url && url.host === 'docs.google.com') {
      ...
      log('internalHeadingId', internalHeadingId)
...
export default function fixGoogleHtml () {
  return (tree, _file) => {
    ...
    fixChecklists(tree);
    fixInternalLinks(tree, _file.data.rawInternalContent)
    return tree;
  };
}
