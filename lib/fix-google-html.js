 ...
 * the entire pagraph in a `<pre><code>` node. Merge consecutive code paragraphs
 * into a single `<pre><code>` block.
 * @param {RehypeNode} node Fix the tree below this node
 */
export function createCodeBlocks(node) {
  const isAllTextIsCode = (parent) => {
    ...
    for (const child of parent.children) {
      ...
      else if (isAllTextIsCode(child) === false) {
        return false;
      }
    }
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (isBlock(child)) {
      if (isAllTextIsCode(child)) {
        // FIXME: should unwrap <code> nodes in child!
        const replacement = {
          type: 'element',
          tagName: 'pre',
          children: [{
  ...
  // FIXME: merge consecutive blocks!
}
