...
function parseStyleProperties(styleText) {
  if (!styleText) return {};
  ...
  const properties = styleText
    ...
    .map(property => {
      ...
      try {
        const {name, value} = property.match(/^\s*(?<name>[\w\-]+):\s*(?<value>.+)\s*$/).groups;
        return [name, value]
      } catch(error) {
...
function getStyle(propertyName, node) {
  node._parsedStyle ||= parseStyleProperties(node.properties?.style);
  const value = node._parsedStyle[propertyName];
 ...
 * @param {string} propertyName
 * @param {RehypeNode} node
 * @param {RehypeNode[]} parents
 * @returns {string|undefined}
 */
function getResolvedStyle(propertyName, node, parents) {
  node._resolvedStyle ||= {};
  if (propertyName in node._resolvedStyle) {
    return node._resolvedStyle[propertyName];
  }

  let value = getStyle(propertyName, node);
  if (value || !parents) {
  ...
  for (let i = parents.length - 1; i >= 0; i--) {
    value = getStyle(propertyName, parents[i]);
    if (value) {
      node._resolvedStyle[propertyName] = value;
      return value;
    }
...
class StyleResolver {
  ...
  get (propertyName) {
    return getResolvedStyle(propertyName, this.node, this.parents);
  }
}
...
function convertInlineStylesToElements(node) {
  visitParents(node, n => isStyled(n) && !isBlock(n), (node, parents) => {
    const style = new StyleResolver(node, parents);
    if (style.get('font-style')?.toLowerCase() === 'italic') {
    ...
    if (verticalAlign?.startsWith('super')) {
      wrapChildren(node, hast('sup'));
    }
    else if (verticalAlign?.startsWith('sub')) {
    ...
    const decorationLine = style.get('text-decoration') || style.get('text-decoration-line');
...
function getNodeTextAlignment (node) {
  const style = new StyleResolver(node);
  const alignMatch = style.get('text-align')?.match(/^(left|center|right)/);
  if (alignMatch) {
    return alignMatch[1];
  }
  return null;
